...
const invoiceSchema = {
    ...
    properties: {
        ...
        invoiceDate: { type: Type.STRING, description: 'The date the invoice was issued, in YYYY-MM-DD format. If the text says "today", use today\'s date.' },
...
export const parseInvoice = async (input: File[] | string): Promise<Partial<Invoice>> => {
    ...
    const prompt = `
        ...
        - vendorName: The name of the company or person that was paid. If the text says "received from [Name]" or "Paid to [Name]", that name is the vendor.
